...
const callReport = () => budgetResolvers.Query.budgetReport(undefined, { projectId: 1 }, ctx);
...
describe('aggregation of records with the same name', () => {
  it('sums multiple expenses with the same name into one line', async () => {
    ...
    (mockIncomes.findMany as jest.Mock).mockResolvedValue([{ name: 'rent', amount: 1500 }]);
  ...
  it('matches names case-insensitively and ignores surrounding whitespace', async () => {
    (mockExpenses.findMany as jest.Mock).mockResolvedValue([{ name: 'Salary', amount: 200 }]);
    (mockIncomes.findMany as jest.Mock).mockResolvedValue([{ name: ' salary ', amount: 3000 }]);
...
describe('names existing only on one side', () => {
  it('sets totalIncome to 0 for an expense with no matching income', async () => {
    (mockExpenses.findMany as jest.Mock).mockResolvedValue([{ name: 'electricity', amount: 100 }]);
